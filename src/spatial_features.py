...
def download_wifi_hotspots(cache_path=None, limit=5000, app_token=None):
    ...
    response = requests.get(WIFI_ENDPOINT, params=params, timeout=30)
...
def download_eateries(cache_path=None, limit=50000, app_token=None):
    ...
    Returns:
        DataFrame with eatery locations, or an empty DataFrame if the
        download fails (e.g. HTTP 403 Forbidden without an app token).
    """
    if cache_path and os.path.exists(cache_path):
        return pd.read_csv(cache_path)
    ...
    if app_token:
        params["$$app_token"] = app_token
    response = requests.get(EATERIES_ENDPOINT, params=params, timeout=30)

    if response.status_code != 200:
        print(
            f"WARNING: Eatery data unavailable (HTTP {response.status_code}). "
            "Eatery penalty will not be applied. Scores may be incomplete."
        )
        return pd.DataFrame()
